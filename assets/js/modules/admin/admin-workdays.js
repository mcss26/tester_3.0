...
(async function () {
  ...
  function getStatusDisplay(status) {
    return (
      STATUS_DISPLAY[status] || { label: status || "—", cls: "status-muted" }
    );
  }
  ...
  function bindEvents() {
    ...
    function changeDateByOffset(offset) {
      const base = ui.inputDate.value
        ? new Date(ui.inputDate.value + "T12:00:00")
        : new Date();
    ...
    ui.staffContainer?.addEventListener("change", (e) => {
      if (e.target.dataset.action === "assign-user") {
        ...
        // Ensure array size
        while (state.allocations[roleId].length <= index)
          state.allocations[roleId].push({});
    ...
    ui.costsContainer?.addEventListener("input", (e) => {
      if (e.target.dataset.costId) {
        ...
        state.costsPlan[costId] = {
          amount,
          isAdjusted: amount !== (def?.base_amount || 0),
        };
        calculateTotals();
      }
    });
    ...
    ui.btnCancelCloseNight?.addEventListener("click", () =>
    ...
    ui.btnCancelPreFlight?.addEventListener("click", () =>
    ...
    ui.btnCancelTemplateModal?.addEventListener("click", () =>
    ...
    // ── Stock Audit Filter ──
    ui.saFilterClasif?.addEventListener("change", () =>
      renderVarianceTable(ui.saFilterClasif.value),
    );
    ...
    document.querySelectorAll("#panelEvento .btn-import").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        const fileInput = document.getElementById(
          e.currentTarget.dataset.trigger,
        );
    ...
    bindFileHandler("file-extracciones", async (f) => {
      const { count } = await window.ImporterExtracciones.process(
        f,
        state.activeWorkDay?.id,
      );
      window.Toast.success(`Importados ${count} retiros.`);
      loadCierreData();
    });
    bindFileHandler("file-gbol", async (f) => {
      const { count } = await window.ImporterGbol.process(
        f,
        state.activeWorkDay?.id,
      );
    ...
    bindFileHandler("file-passline", async (f) => {
      const { count } = await window.ImporterPassline.process(
        f,
        state.activeWorkDay?.id,
      );
  ...
  function bindFileHandler(inputId, handler) {
    ...
    input.addEventListener("change", async (e) => {
      ...
      try {
        if (!state.activeWorkDay?.id && inputId !== "file-afip")
  ...
  async function loadInitialData() {
    const loadCtrl = window.Utils.setPageState(ui, { loading: true });
    try {
      ...
      state.openingCosts.forEach(
        (c) =>
          (state.costsPlan[c.id] = {
  ...
  async function handleDateChange() {
    ...
    if (ui.currentDateDisplay) {
      ...
      ui.currentDateDisplay.textContent = formatted;
      const mirror = document.getElementById('input-date-mirror');
  ...
  async function loadDayDetails(dayId) {
    // 1. Planning
    const { data: planning } = await window.sb
    ...
    const { data: convos } = await window.sb
    ...
    // 3. Metadata
    if (state.activeWorkDay.notes)
      ui.inputNotes.value = state.activeWorkDay.notes;
    // Auto-select event if date matches (work_days HAS event_id & event_name columns)
    const matchingEvent = state.events.find(
      (ev) => ev.date === state.activeWorkDay.work_date,
    );
  ...
  function renderEventsDropdown() {
    ...
    const options = state.events
      ...
      .join("");
    ui.selectEvent.innerHTML =
      '<option value="">-- Sin Evento Vinculado --</option>' + options;
  }

  function renderStaffList() {
    if (!ui.staffContainer) return;

    ui.staffContainer.innerHTML = state.roles
      .map((role) => {
    ...
    if (state.activeWorkDay) {
      state.roles.forEach((role) =>
        renderStaffSlots(role.id, state.staffPlan[role.id]),
      );
    }
  }
  ...
  function renderStaffSlots(roleId, qty) {
    ...
    const userOptions = eligibleUsers
      .map(
        (u) =>
          `<option value="${u.id}">${window.Utils.escapeHtml(u.full_name)}</option>`,
      )
      .join("");
  ...
  function renderCostsList() {
    ...
    ui.costsContainer.innerHTML = state.openingCosts
      .map((cost) => {
  ...
  function calculateTotals() {
    ...
    state.roles.forEach((role) => {
      ...
      // Update badge
      const input = ui.staffContainer.querySelector(
    ...
    Object.values(state.costsPlan).forEach(
      (plan) => (fixedTotal += plan.amount),
    );
  ...
  async function handleCreate() {
    ...
    const confirmed = await window.Utils.confirmAction(
      `¿Crear jornada para ${window.WorkDayHelper.formatDate(dateVal)}?`,
      { confirmText: "Crear" },
    );
    if (!confirmed) return;
  ...
  function showPreFlightModal() {
    ...
    // Render checklist
    ui.preFlightChecks.innerHTML = checks
      .map(
        (c) => `
  ...
  function runPreFlightChecks(wd) {
    ...
    const staffCards =
      ui.staffContainer?.querySelectorAll(".planner-item") || [];
    ...
    checks.push({
      ...
      info:
        assignedStaff > 0
          ? `${assignedStaff} persona(s) asignada(s)`
          : "Sin staff asignado",
    ...
    checks.push({
      ...
      info:
        costCards.length > 0
          ? `${costCards.length} costo(s) cargado(s)`
          : "Sin costos — procederá con $0",
    ...
    checks.push({
      ...
      info: hasEvent
        ? ui.selectEvent.options[ui.selectEvent.selectedIndex]?.text || "Sí"
        : "Sin evento — noche genérica",
      pass: hasEvent,
      critical: false,
    });
  ...
  async function handlePreFlightConfirm() {
    ...
    window.Utils.setPageState(ui, { loading: true });
    try {
      await window.sb.rpc("rpc_open_work_day", {
        p_work_day_id: state.activeWorkDay.id,
      });
  ...
  function updateFooterButtons(status) {
    const btnSave =
      ui.btnSavePlanning || document.getElementById("btn-save-planning");
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      const staffPayload = state.roles.map((role) => ({
        ...
        approved_budget:
          (state.staffPlan[role.id] || 0) * (role.base_rate || 0),
      }));
      await window.sb
        .from("work_day_staff_planning")
        .upsert(staffPayload, { onConflict: "work_day_id, role_id" });
  ...
  async function handleCreateEvent() {
    ...
    try {
      ...
      const qrQty =
        parseInt(document.getElementById("input-event-qr-qty")?.value) || 0;
      if (qrQty > 0) {
        const { data: batch, error: batchErr } = await window.sb
          .from("qr_batches")
          .insert({
            name: `${name} - Auto`,
    ...
    try {
      ...
      if (!closing) {
        const { data: newC, error: createErr } = await window.sb
          ...
          .single();
        if (createErr) {
          window.Toast.error("No se pudo crear cierre de caja.");
          return;
        }
        closing = newC;
      ...
      ui.btnCloseNight.textContent =
        closing.status === STATUS.CLOSED ? "CERRADO" : "CERRAR NOCHE";
  ...
  function renderCierreTable(terminals, details) {
    ...
    const rows = terminals
      .map((t) => {
        const d = details.find((x) => x.terminal_id === t.id) || {
          declared_cash: 0,
          declared_zoco: 0,
          system_cash: 0,
          system_zoco: 0,
        ...
        const cD = Number(d.declared_cash) || 0,
          zD = Number(d.declared_zoco) || 0;
        const cS = Number(d.system_cash) || 0,
          zS = Number(d.system_zoco) || 0;
        ...
        const si =
          cierreStatusLabels[(d.status || "pending").toLowerCase()] ||
          cierreStatusLabels.pending;
        const dc =
          diff === 0 ? "muted" : diff < 0 ? "text-error" : "text-success";
    ...
    ui.cierreTableBody.innerHTML =
      rows ||
      '<tr><td colspan="7" class="cell-pad text-center muted">Sin terminales</td></tr>';
  ...
  function renderCierreTotals(acc) {
    ...
    if (ui.evtKpiSystem)
      ui.evtKpiSystem.textContent = window.Utils.formatARS(totalSys);
    if (ui.evtKpiDeclared)
      ui.evtKpiDeclared.textContent = window.Utils.formatARS(totalDecl);
    if (ui.evtKpiDiff) {
      ui.evtKpiDiff.textContent = window.Utils.formatARS(acc.diff);
      applyDiffClass(ui.evtKpiDiff, acc.diff);
    }
  }
  ...
  async function loadQrStats(workDayId) {
    ...
    ui.qrBoleteria.sys.textContent = window.Utils.formatARS(
      stats.boleteria.sys,
    );
  ...
  function renderBreakdown(s) {
    ...
    const barCash = s.bar_sales_cash || 0,
      barCard = s.bar_sales_card || 0;
    const barTotal = s.bar_sales_system || barCash + barCard;
    const qrTotal = s.qr_total || 0;
    const totalCash = barCash,
      totalZoco = barCard + qrTotal,
      globalTotal = totalCash + totalZoco;

    const fmt = window.Utils.formatARS;
    if (el("breakdown-bar-cash"))
      el("breakdown-bar-cash").textContent = fmt(barCash);
    if (el("breakdown-bar-card"))
      el("breakdown-bar-card").textContent = fmt(barCard);
    if (el("breakdown-bar-total"))
      el("breakdown-bar-total").textContent = fmt(barTotal);
    if (el("breakdown-qr-zoco"))
      el("breakdown-qr-zoco").textContent = fmt(qrTotal);
    if (el("breakdown-qr-total"))
      el("breakdown-qr-total").textContent = fmt(qrTotal);
    if (el("breakdown-total-cash"))
      el("breakdown-total-cash").textContent = fmt(totalCash);
    if (el("breakdown-total-zoco"))
      el("breakdown-total-zoco").textContent = fmt(totalZoco);
    if (el("breakdown-total-global"))
      el("breakdown-total-global").textContent = fmt(globalTotal);
  }
  ...
  async function openCloseNightModal() {
    ...
    try {
      const { data: preflight, error: pfErr } = await window.sb.rpc(
        "rpc_preflight_close_workday",
        {
          p_work_day_id: state.activeWorkDay.id,
        },
      );
      if (pfErr) {
        window.Toast.error(pfErr.message);
        return;
      }
      ...
      window.Toast.error(
        "Error al ejecutar pre-flight: " + (err.message || err),
      );
      return;
    }
  ...
  function renderPreflightChecks(checks) {
    ...
    if (!checks || checks.length === 0) {
      ui.confirmDiffDisplay.innerHTML =
        '<span class="muted">Sin datos de validación</span>';
      return;
    }
  ...
  function renderFinancialSummary(summary) {
    ...
    if (ui.evtKpiSystem)
      ui.evtKpiSystem.textContent = fmt(summary.total_system || 0);
    if (ui.evtKpiDeclared)
      ui.evtKpiDeclared.textContent = fmt(summary.total_declared || 0);
    if (ui.evtKpiDiff) {
    ...
    if (ui.totalCashSys)
      ui.totalCashSys.textContent = fmt(summary.cash_system || 0);
    if (ui.totalZocoSys)
      ui.totalZocoSys.textContent = fmt(summary.zoco_system || 0);
    if (ui.totalCashDecl)
      ui.totalCashDecl.textContent = fmt(summary.cash_declared || 0);
    if (ui.totalZocoDecl)
      ui.totalZocoDecl.textContent = fmt(summary.zoco_declared || 0);
    if (ui.totalDiff) {
  ...
  async function _populatePnlModal() {
    ...
    try {
      ...
      if (pnl) {
        ...
        const bePct =
          totalExpense > 0
            ? Math.min((totalIncome / totalExpense) * 100, 150)
            : 0;
  ...
  async function performCloseNight() {
    ...
    try {
      ...
      const { data: closeResult, error: closeErr } = await window.sb.rpc(
        "rpc_close_work_day",
        {
          p_work_day_id: state.activeWorkDay.id,
  ...
  async function handleSaveNotes() {
    ...
    try {
      await window.sb
        .from("cash_closings")
        .update({ notes: ui.closingNotes.value.trim() })
        .eq("id", state.closingId);
      window.Toast.success("Notas guardadas.");
    } catch (e) {
      window.Toast.error("Error guardando notas.");
    }
  }
  ...
  async function pollKPIs() {
    ...
    try {
      ...
      if (closing) {
        if (ui.evtKpiSystem)
          ui.evtKpiSystem.textContent = window.Utils.formatARS(
            closing.total_system || 0,
          );
        if (ui.evtKpiDeclared)
          ui.evtKpiDeclared.textContent = window.Utils.formatARS(
            closing.total_declared || 0,
          );
        if (ui.totalDiff) {
      ...
      if (ui.liveChipTime) {
        const now = new Date();
        ui.liveChipTime.textContent = now.toLocaleTimeString("es-AR", {
          hour: "2-digit",
          minute: "2-digit",
          second: "2-digit",
        });
  ...
  async function loadAccruals() {
    ...
    try {
      const { data, error } = await window.sb
        .from("staff_accruals")
        .select(
          "id, work_day_id, user_id, role_id, base_amount, adjustments, status, created_at, profiles(full_name), master_staff_roles(name)",
        )
        .eq("work_day_id", state.activeWorkDay.id)
        .order("created_at");
      ...
      if (ui.btnGenerateAccruals) {
        ...
        ui.btnGenerateAccruals.disabled =
          state.accruals.length > 0 && !hasAccrued;
        ui.btnGenerateAccruals.textContent =
  ...
  function renderAccruals() {
    ...
    if (!state.accruals || state.accruals.length === 0) {
      ...
      if (ui.devencionesTotalFooter)
        ui.devencionesTotalFooter.textContent = "$0";
      return;
    }
    ...
    let totalAccrued = 0;
    ui.devencionesTableBody.innerHTML = state.accruals
      .map((a) => {
    ...
    // Totals
    if (ui.devencionKpiTotal)
      ui.devencionKpiTotal.textContent = fmt(totalAccrued);
    if (ui.devencionesTotalFooter)
      ui.devencionesTotalFooter.textContent = fmt(totalAccrued);

    // Bind adjustment inputs
    ui.devencionesTableBody.querySelectorAll(".js-adj-input").forEach((inp) => {
      inp.addEventListener("change", () =>
        adjustAccrual(inp.dataset.id, parseFloat(inp.value) || 0),
      );
    });
  }
  ...
  async function generateAccruals() {
    ...
    try {
      ...
      const { data, error } = await window.sb.rpc(
        "admin_generate_workday_accruals",
        {
          p_work_day_id: state.activeWorkDay.id,
        },
      );
      ...
      if (result?.new_accruals > 0) {
        window.Toast.success(
          `${result.new_accruals} devencion(es) generada(s).`,
        );
      } else {
        window.Toast.info(
          "No se generaron nuevas devenciones (ya existían o no hay convocados confirmados).",
        );
      }
  ...
  async function handleGbolSync() {
    ...
    try {
      ...
      await window.GbolService.populateSystemAmounts(
        state.activeWorkDay.id,
        dateStr,
      );
  ...
  async function loadFiscalSummary(dateStr) {
    ...
    try {
      ...
      if (data) {
        ...
        if (ui.fiscalBruto)
          ui.fiscalBruto.textContent = window.Utils.formatARS(
            data.total_bruto || 0,
          );
        if (ui.fiscalPctBlanco)
          ui.fiscalPctBlanco.textContent = `${data.pct_blanqueado || 0}%`;
        if (ui.fiscalIva)
          ui.fiscalIva.textContent = window.Utils.formatARS(
            data.total_iva || 0,
          );
        if (ui.fiscalTickets)
          ui.fiscalTickets.textContent = data.total_tickets || 0;
      } else {
  ...
  async function loadStockAuditData() {
    ...
    try {
      ...
      // KPIs
      const totPhysical = state.barEfficiency.reduce(
        (s, r) => s + Number(r.cost_physical || 0),
        0,
      );
      const totTheoretical = state.barEfficiency.reduce(
        (s, r) => s + Number(r.cost_theoretical || 0),
        0,
      );
      const totLoss = state.barEfficiency.reduce(
        (s, r) => s + Number(r.loss_amount || 0),
        0,
      );
      const avgRating = state.barEfficiency.length
        ? state.barEfficiency.reduce(
            (s, r) => s + Number(r.efficiency_pct || 100),
            0,
          ) / state.barEfficiency.length
        : 0;

      if (ui.saKpiPhysical)
        ui.saKpiPhysical.textContent = window.Utils.formatARS(totPhysical);
      if (ui.saKpiTheoretical)
        ui.saKpiTheoretical.textContent =
          window.Utils.formatARS(totTheoretical);
      if (ui.saKpiLoss)
        ui.saKpiLoss.textContent = window.Utils.formatARS(totLoss);
      if (ui.saKpiLossDelta) {
        const pctLoss =
          totTheoretical > 0
            ? ((totLoss / totTheoretical) * 100).toFixed(1)
            : 0;
  ...
  function renderSessionsTable() {
    ...
    if (!state.barEfficiency.length) {
      ui.saSessionsBody.innerHTML =
        '<tr><td colspan="8" class="cell-pad text-center muted">Sin sesiones de barra para esta noche</td></tr>';
      return;
    }

    ui.saSessionsBody.innerHTML = state.barEfficiency
      .map((s) => {
        const varPct = s.efficiency_pct
          ? (100 - Number(s.efficiency_pct)).toFixed(1)
          : "—";
  ...
  function renderVarianceTable(filterClasif) {
    ...
    if (filterClasif)
      rows = rows.filter((r) => r.clasificacion === filterClasif);

    if (!rows.length) {
      ui.saVarianceBody.innerHTML =
        '<tr><td colspan="9" class="cell-pad text-center muted">Sin datos de varianza</td></tr>';
      return;
    }
    ...
    ui.saVarianceBody.innerHTML = rows
      .map((r) => {
  ...
  function renderConsumoTeorico() {
    ...
    if (!state.consumoTeorico.length) {
      ui.saConsumoGbolBody.innerHTML =
        '<tr><td colspan="4" class="cell-pad text-center muted">Sin datos GBOL para esta noche</td></tr>';
      return;
    }

    ui.saConsumoGbolBody.innerHTML = state.consumoTeorico
      .map(
        (c) => `
            <tr class="table-row">
  ...
  async function renderHistoryTable() {
    ...
    try {
      const { data, error } = await window.sb
        ...
        .select(
          "work_date, event_name, status, total_income, gbol_efectivo, gbol_efectivo_neto, total_retiros, cant_retiros, cash_declared, conciliacion_diff, stock_loss, staff_cost, net_result, health_score",
        )
      ...
      if (!data || !data.length) {
        ui.historyTableBody.innerHTML =
          '<tr><td colspan="13" class="p-4 text-center muted italic">Sin jornadas registradas</td></tr>';
        return;
      }
      ...
      const diffCell = (val) => {
        if (val == null)
          return '<td class="table-cell text-right font-mono muted">—</td>';
        const n = Number(val);
      ...
      ui.historyTableBody.innerHTML = data
        .map((row) => {
      ...
      ui.historyTableBody.innerHTML =
        '<tr><td colspan="13" class="p-4 text-center text-danger">Error cargando historial</td></tr>';
    }
  }
  ...
  async function loadReportDashboard() {
    ...
    try {
      ...
      const [pnlRes, hsRes, effRes, closingRes, accRes, fiscalRes, histRes] =
        await Promise.all([
          window.sb
            .from("vw_workday_pnl")
            .select("*")
          ...
          window.sb
            .from("staff_accruals")
            .select("id, status, base_amount, adjustments")
            .eq("work_day_id", wdId),
          window.sb
            .from("vw_fiscal_summary")
            .select("*")
          ...
          window.sb
            .from("vw_night_snapshot")
            .select("total_income, net_result, health_score")
            .order("work_date", { ascending: false })
            .limit(10),
        ]);
      ...
      const closing = closingRes.data;
      const accruals = (accRes.data || []).filter(
  ...
  function loadReportHeader(dateStr, healthScore) {
    ...
    if (ui.rptEvent)
      ui.rptEvent.textContent =
        state.activeWorkDay?.event_name ||
        state.activeWorkDay?.events?.name ||
        "—";

    if (ui.rptHealthBadge && healthScore != null) {
      const score = Number(healthScore);
      ui.rptHealthBadge.textContent = `${score}/100`;
      ui.rptHealthBadge.className =
        "status-pill " +
        (score >= 75
          ? "staff-status-active"
          : score >= 50
  ...
  function loadReportKpis(pnl, healthScore, history) {
    ...
    const perCapita = pnl.qr_count
      ? Math.round(totalIncome / pnl.qr_count)
      : null;
    ...
    if (ui.rptKpiMargin)
      ui.rptKpiMargin.textContent =
        margin != null ? `${margin.toFixed(1)}%` : "—";
    if (ui.rptKpiPercapita)
      ui.rptKpiPercapita.textContent = perCapita ? fmt(perCapita) : "—";
    if (ui.rptKpiScore) {
    ...
    if (history.length > 1) {
      ...
      setDelta(
        ui.rptKpiRevenueDelta,
        totalIncome,
        avg(history, "total_income"),
      );
      ...
      options: {
        ...
        plugins: {
          legend: {
            display: true,
            position: "bottom",
            labels: { boxWidth: 12, padding: 16, color: "#9ca3af" },
          },
        },
        scales: {
          x: {
          ...
          y: {
            ticks: {
              color: "#6b7280",
              font: { size: 10 },
              callback: (v) => window.Utils.formatARS(v),
            },
  ...
  function renderReportFiscal(fiscal) {
    ...
    if (!fiscal) {
      ui.rptFiscalBody.innerHTML =
        '<tr><td colspan="4" class="p-4 text-center muted italic">Sin datos fiscales para esta noche</td></tr>';
      return;
    }
  ...
  function renderReportAnomalies(closing, efficiency) {
    ...
    if (efficiency.length) {
      const totalLoss = efficiency.reduce(
        (s, r) => s + Number(r.loss_amount || 0),
        0,
      );
      const avgEff =
        efficiency.reduce((s, r) => s + Number(r.efficiency_pct || 100), 0) /
        efficiency.length;
    ...
    if (anomalies.length === 0) {
      ui.rptAnomaliesList.innerHTML =
        '<div class="rpt-anomaly-empty muted italic">Sin anomalías detectadas ✅</div>';
      return;
    }

    ui.rptAnomaliesList.innerHTML = anomalies
      .map(
        (a) => `
  ...
  function renderReportOps(efficiency, closing, accruals) {
    ...
    if (efficiency.length) {
      ...
      const skusDiff = efficiency.filter(
        (r) => Math.abs(Number(r.loss_amount || 0)) > 0,
      ).length;

      if (ui.rptOpsStockPrecision)
        ui.rptOpsStockPrecision.textContent = `${avgEff.toFixed(0)}%`;
    ...
    // Nómina
    if (accruals.length) {
      const totalCost = accruals.reduce(
        (s, a) => s + Number(a.base_amount || 0) + Number(a.adjustments || 0),
        0,
      );
      if (ui.rptOpsNominaCount)
        ui.rptOpsNominaCount.textContent = String(accruals.length);
  ...
  async function loadTemplates() {
    try {
      const { data, error } = await window.sb
        ...
        .select(
          "id, name, staff_config, cost_ids, avg_revenue, avg_attendance, usage_count",
        )
  ...
  async function handleSaveTemplate() {
    ...
    if (!name) {
      window.Toast.error("Ingresá un nombre.");
      return;
    }
  ...
  function renderBenchmarkPills() {
    if (!ui.benchmarkPills || !state.benchmarks) {
      if (ui.benchmarkPills)
        ui.benchmarkPills.innerHTML =
          '<span class="wd-benchmark-pill muted">Sin datos históricos</span>';
      return;
    }
