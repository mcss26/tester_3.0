...
SELECT
    'T1: cash_closings → work_days FK' as test,
    CASE
        WHEN COUNT(*) = 0 THEN 'PASS'
        ELSE 'FAIL: ' || COUNT(*) || ' huérfanas'
    END as result
FROM
    cash_closings cc
...
SELECT
    'T2: closing_terminals → cash_closings FK',
    CASE
        WHEN COUNT(*) = 0 THEN 'PASS'
        ELSE 'FAIL: ' || COUNT(*) || ' huérfanas'
    END
FROM
    closing_terminals ct
...
SELECT
    'T3: closing_terminals → pos_terminals FK',
    CASE
        WHEN COUNT(*) = 0 THEN 'PASS'
        ELSE 'FAIL: ' || COUNT(*) || ' terminales inválidas'
    END
FROM
    closing_terminals ct
...
SELECT
    'T4: No closings abiertos con jornada cerrada',
    CASE
        WHEN COUNT(*) = 0 THEN 'PASS'
...
WHERE
    cc.status IN ('open', 'pending')
    AND UPPER(wd.status) = 'CLOSED'
UNION ALL

-- TEST 5: total_difference = total_declared - total_system (signo correcto)
SELECT
    'T5: total_difference = declared - system',
    CASE
        WHEN COUNT(*) = 0 THEN 'PASS'
...
WHERE
    LOWER(cc.status) = 'closed'
    AND cc.total_difference != (
        cc.total_declared - cc.total_system
    )
UNION ALL

-- TEST 6: conciliacion_diff en vista = declared - system (mismo signo que cash_closings)
SELECT
    'T6: conciliacion_diff signo consistente con cash_closings',
    CASE
        WHEN COUNT(*) = 0 THEN 'SKIP: no hay datos para comparar'
        WHEN COUNT(*) FILTER (
            WHERE
                sign(vns.conciliacion_diff) != sign(cc.total_difference)
                AND cc.total_difference != 0
        ) = 0 THEN 'PASS'
...
SELECT
    'T7: cash_movements → cash_closings FK',
    CASE
        WHEN COUNT(*) = 0 THEN 'PASS'
        ELSE 'FAIL: ' || COUNT(*) || ' movimientos huérfanos'
    END
FROM
    cash_movements cm
...
SELECT
    'T8: total_retiros en snapshot = SUM(cash_movements)',
    CASE
        WHEN COUNT(*) = 0 THEN 'SKIP: no hay retiros'
        WHEN COUNT(*) FILTER (
            WHERE
                abs(
                    vns.total_retiros - COALESCE(cm_sum.total, 0)
                ) > 0.01
        ) = 0 THEN 'PASS'
        ELSE 'FAIL: retiros no coinciden en ' || COUNT(*) FILTER (
            WHERE
                abs(
                    vns.total_retiros - COALESCE(cm_sum.total, 0)
                ) > 0.01
...
SELECT
    'T9: gbol_efectivo = SUM(closing_terminals.system_cash)',
    CASE
        WHEN (
            SELECT COUNT(*)
            FROM closing_terminals
        ) = 0 THEN 'SKIP: no hay terminales'
        WHEN COUNT(*) FILTER (
            WHERE
                abs(
                    vns.gbol_efectivo - COALESCE(ct_sum.total_sys_cash, 0)
                ) > 0.01
        ) = 0 THEN 'PASS'
        ELSE 'FAIL: gbol_efectivo no coincide en ' || COUNT(*) FILTER (
            WHERE
                abs(
                    vns.gbol_efectivo - COALESCE(ct_sum.total_sys_cash, 0)
                ) > 0.01
