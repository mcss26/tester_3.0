# Remediacion Golden Standard: admin-reportes.html

## Contexto
Archivo: pages/admin/admin-reportes.html
Score actual: 78/100
Modulo: admin

## Estado actual del componente

### Compliance por categoria (solo relevantes):
  * Layout: pass (100%) -- Completo
  * Navigation: pass (100%) -- Completo
  * Header: parcial (60%) -- Missing: dashboard-title-soft, actions-bar
  * TabSystem: parcial (67%) -- Missing: tab-content
  * Tables: parcial (75%) -- Missing: cell-pad, sortable, sort-icon
  * Buttons: parcial (50%) -- Missing: btn-primary, btn-secondary, btn-icon-plus, btn-danger
  * Forms: parcial (50%) -- Missing: form-group, form-label, date-range-inline
  * Utilities: falta (0%) -- Missing: u-hidden, u-visible, text-xs, text-muted, badge, badge-quiet

### Anti-patrones detectados:
  * Inline styles: 0
  * Native select sin custom-dropdown: 0
  * Cobertura ARIA total: 3 attrs

### Elementos HTML:
  * input: 2
  * button: 8
  * table: 5

### Headings: h2(1) h3(2)

### Hints de remediacion (priorizados):
  * MED: [Header] Agregar: dashboard-title-soft, actions-bar
  * MED: [TabSystem] Agregar: tab-content
  * MED: [Tables] Agregar: cell-pad, sortable, sort-icon
  * LOW: [Buttons] Agregar: btn-primary, btn-secondary, btn-icon-plus, btn-danger
  * LOW: [Forms] Agregar: form-group, form-label, date-range-inline
  * LOW: [Utilities] Agregar: u-hidden, u-visible, text-xs, text-muted, badge, badge-quiet

## Instrucciones

Crea un plan de implementacion para remediar admin-reportes.html al Golden Standard.

### Reglas:
1. Consulta docs/ui-golden-standard.md como referencia absoluta
2. Referencia de implementacion: pages/admin/admin-central-stock.html
3. Solo modifica HTML y CSS. NO toques logica JS (Supabase, state, event handlers)
4. Usa clases de components.css, no inventes clases nuevas
5. Si la pagina tiene CSS propio (ej: assets/css/admin-reportes.css), refactoriza ahi
6. Elimina TODOS los inline style y reemplazalos con clases GS
7. Reemplaza select nativos con el patron .custom-dropdown
8. Asegura heading hierarchy correcta (h2, h3, h4 -- sin h1)
9. Agrega atributos ARIA a elementos interactivos

### Formato del plan:
Para cada archivo a modificar, indica:
  * Que cambia y por que
  * Lineas aproximadas afectadas
  * Patron GS de referencia (numero de seccion del golden standard)

### Criterio de exito:
  * Score de compliance mayor o igual a 85%
  * 0 inline styles
  * 0 native selects sin custom-dropdown
  * ARIA labels en todos los botones e inputs
